...
function renderTasks(tasks) {
  ...
  tasks.forEach((task) => {
    ...
    editIcon.className = `fa-solid fa-pen text-gray-600 cursor-pointer text-xl ${task.status ? "!hidden" : ""}`;
    editIcon.addEventListener("click", () =>
      editTask(task.id, task.name, task.status),
    );

    const checkIcon = document.createElement("i");
    checkIcon.className = `fa-regular  cursor-pointer text-xl mr-2 ${task.status ? "fa-square-check text-secondary" : "fa-square text-gray-600"}`;
    checkIcon.addEventListener("click", () => {
      completeTask(task.id, task.name, task.status);
    });

    const deleteIcon = document.createElement("i");
    deleteIcon.className = `fa-regular fa-trash-can text-gray-600 cursor-pointer rounded text-xl ${task.status ? "" : "!hidden"}`;
